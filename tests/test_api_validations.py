...
from calls.models import EvaluationForm, CallQueue, CallRecord, Evaluation
...
class TestAPIValidations:
    ...
    def test_agent_permissions(self, setup_users, setup_form):
        ...
        form_data = {
            "name": "Agent Form",
            "fields": [{"key": "test", "label": "Test", "type": "number", "max_score": 10}]
        }
        response = self.client.post("/api/evaluation-forms/", form_data, format='json')
    ...
    def test_invalid_form_data(self, setup_users):
        ...
        response = self.client.post("/api/evaluation-forms/", invalid_form, format='json')
        assert response.status_code == 400
