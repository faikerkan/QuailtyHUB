...
from calls.models import EvaluationForm, CallQueue
import datetime
import io
...
class TestAPIEndToEnd:
    ...
    def test_complete_evaluation_flow(self, setup_test_data):
        ...
        form_response = self.client.post("/api/evaluation-forms/", form_data, format='json')
        ...
        call_response = self.client.post("/api/calls/upload/", call_data, format='multipart')
        ...
        calls = calls_response.data['results'] if 'results' in calls_response.data else calls_response.data
        assert len(calls) == 1
        assert calls[0]['phone_number'] == "5551234567"

        # Değerlendirme sonuçlarını kontrol et
        evals_response = self.client.get(f"/api/evaluations/{eval_response.data['id']}/")
    ...
    def test_multiple_evaluations_flow(self, setup_test_data):
        ...
        assert evals_response.status_code == 200
        evals = evals_response.data['results'] if 'results' in evals_response.data else evals_response.data
        assert len(evals) == 3
