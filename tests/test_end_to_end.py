...
class TestEndToEnd:
    ...
    def test_basic_evaluation_flow(self, client, setup_test_data):
        ...
        assert my_evaluations.first().total_score == Decimal(
            "86.67"
        )  # (8/10 + 18/20) * 100 / 2
    ...
    def test_multiple_agents_flow(self, client, setup_test_data):
        ...
        agent2 = CustomUser.objects.create_user(
            username="agent2test", password="agent123", role="agent"
        )
        ...
        for agent in [self.agent, agent2]:
            for i in range(2):
                # Çağrı oluştur
                audio = SimpleUploadedFile(
                    f"test_{agent.username}_{i}.mp3",
                    b"file_content",
        ...
        for agent in [self.agent, agent2]:
            ...
            agent_evals = Evaluation.objects.filter(call__agent=agent).order_by(
                "evaluated_at"
            )
    ...
    def test_form_versioning_flow(self, client, setup_test_data):
        ...
        for form in [form_v1, form_v2]:
            # Çağrı oluştur
            audio = SimpleUploadedFile(
                f"test_{form.name}.mp3", b"file_content", content_type="audio/mp3"
            )
            call = CallRecord.objects.create(
                uploaded_by=self.expert,
                agent=self.agent,
        ...
        # Değerlendirmeleri kontrol et
        evaluations = Evaluation.objects.filter(call__agent=self.agent).order_by(
            "form__name"
        )
