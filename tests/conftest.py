...
def create_test_users_and_data(db, tmp_path):
    ...
    if not User.objects.filter(username="kalitetest").exists():
        kalite = User.objects.create_user(
            username="kalitetest",
            password="kalite123",
    ...
    queue, _ = CallQueue.objects.get_or_create(
        name="Test Kuyruğu", defaults={"description": "Test için otomatik oluşturuldu."}
    )
    ...
    call, _ = CallRecord.objects.get_or_create(
        ...
        call_date=call_date,
        uploaded_by=admin,
        defaults={"audio_file": "call_records/test.mp3", "call_id": "TESTCALL1"},
    )
    ...
    if not Evaluation.objects.filter(call=call).exists():
        scores = {
            key: {
                "score": value.get("max_score", 5),
                "max_score": value.get("max_score", 5),
