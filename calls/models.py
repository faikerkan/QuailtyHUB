...
class EvaluationForm(models.Model):
    ...
    def create_default_form(cls, user):
        ...
        default_fields = {
            ...
            "field_7": {"label": "Empati", "type": "number", "max_score": 5},
            "field_8": {
        ...
        return cls.objects.create(
            name="Standart Değerlendirme Formu", created_by=user, fields=default_fields
        )
...
class CallRecord(models.Model):
    ...
    agent = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="agent_calls"
    )
    call_queue = models.ForeignKey(
        CallQueue, on_delete=models.CASCADE, related_name="calls"
    )
...
class Evaluation(models.Model):
    ...
    call = models.ForeignKey(
        CallRecord, on_delete=models.CASCADE, related_name="evaluations"
    )
    evaluator = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="evaluations"
    )
    form = models.ForeignKey(
        EvaluationForm, on_delete=models.CASCADE, related_name="evaluations"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if max_points > 0:
            self.total_score = ((total_points / max_points) * Decimal("100")).quantize(
                Decimal("0.01")
            )
        else:
...
class Call(models.Model):
    ...
    call_type = models.CharField(
        max_length=20, choices=[("inbound", "Gelen"), ("outbound", "Giden")]
    )
    created_at = models.DateTimeField(auto_now_add=True)
