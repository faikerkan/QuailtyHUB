...
class EvaluationFormSerializer(serializers.ModelSerializer):
    ...
    def validate_fields(self, value):
        if not isinstance(value, dict) or not value:
            raise serializers.ValidationError(
                "Alanlar sözlük (dict) olmalı ve boş olmamalı."
            )
        for key, field in value.items():
            if not all(k in field for k in ("label", "type", "max_score")):
                raise serializers.ValidationError(
                    "Her alan 'label', 'type', 'max_score' içermelidir."
                )
            if (
                not isinstance(field["max_score"], (int, float))
                or field["max_score"] <= 0
            ):
                raise serializers.ValidationError(
                    "max_score pozitif bir sayı olmalıdır."
                )
        return value
    ...
    def validate_scores(self, value):
        ...
        for key, field in form_instance.fields.items():
            ...
            if key in value and value[key]["score"] > max_score:
                raise serializers.ValidationError(
                    f"{key} için maksimum puan {max_score} olabilir."
                )
        return value
