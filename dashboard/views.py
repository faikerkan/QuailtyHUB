...
from calls.models import CallRecord, Evaluation, EvaluationForm, CallQueue
...
def admin_dashboard(request):
    ...
    evals_by_day = Evaluation.objects.filter(evaluated_at__date__gte=last_7_days[-1]) \
...
def expert_dashboard(request):
    ...
    evaluated_call_ids = Evaluation.objects.filter(evaluator=request.user).values_list('call_id', flat=True)
    pending_calls = CallRecord.objects.filter(~Q(id__in=Evaluation.objects.values_list('call_id', flat=True))).order_by('-call_date')[:5]
    # Son değerlendirmeler
    recent_evaluations = Evaluation.objects.filter(evaluator=request.user).order_by('-evaluated_at')[:5]
    # İstatistikler
    total_evaluations = Evaluation.objects.filter(evaluator=request.user).count()
    this_month_evaluations = Evaluation.objects.filter(evaluator=request.user, evaluated_at__date__gte=month_start).count()
    avg_score = Evaluation.objects.filter(evaluator=request.user).aggregate(avg=Avg('total_score'))['avg'] or 0
    context = {
...
def agent_dashboard(request):
    ...
    my_evaluations = Evaluation.objects.filter(call__agent=request.user).order_by('-evaluated_at')[:5]
    # Ortalama puan
    avg_score = Evaluation.objects.filter(call__agent=request.user).aggregate(avg=Avg('total_score'))['avg'] or 0
    context = {
...
def admin_reports(request):
    ...
    evals = Evaluation.objects.select_related('call', 'evaluator', 'call__agent', 'call__call_queue')
    if start_date:
    ...
    agent_stats = evals.values('call__agent__id', 'call__agent__first_name', 'call__agent__last_name') \
        .annotate(
            total=Count('id'),
    ...
    expert_stats = evals.values('evaluator__id', 'evaluator__first_name', 'evaluator__last_name') \
        .annotate(
            total=Count('id'),
            avg_score=Avg('total_score'),
            last_eval=Max('evaluated_at')
